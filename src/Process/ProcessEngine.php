...
class ProcessEngine
{
    ...
    public function startNewProcess(){
        ...
        $this->end or Verify::fail("process abnormal end, that means no end event triggered"); //过程没有触发end事件就结束了
    }
    ...
    {
        is_string($nodeName) or Verify::fail(new \InvalidArgumentException("\$nodeName must be a string"));
        is_string($invokeMethod) or Verify::fail(new \InvalidArgumentException("\$invokeMethod must be a string"));
        if (!$context) {
